...
## set seed from within R
seed_env = os.environ.get("SEED")
if seed_env is not None:
    sim.setRNG(seed=int(seed_env, 16))  # Set RNG seed from env
    SEED = seed_env
else:
    SEED = "0x%08x" % sim.getRNG().seed()
...
### changed to store individual runs into separate folders
RESULTS_DIR = Path("./results") / SEED
RESULTS_DIR.mkdir(parents=True, exist_ok=True)


# which generation to begin logging at
#### change back to 1 to include every generation. 
#### AG changed to 81 because he didn't need these files for running tests
BEGIN_LOGGING_AT_GENERATION = 81
...
#with prevelance
mean_C = [16.7, 16.7, 16.7, 20.5, 26.0] 
variance_C = [3.24,3.24,3.24, 2.89, 3.0]
mean_N = [5.5,5.5,5.5, 8.7, 5.0]
variance_N = [0.25,0.25,0.25, 0.49, 0.4]
...
sample_count = 509 #60 #initial sample size 60, trialling 509 to match emperical data
...
genSteps = [staticPhaseEnd, firstDecline,secondDecline, thirdDecline, bottleNeck, firstRecovery,secondRecovery,currentGen,hopefulFuture]

#Population sizes
#Jackson et al 2016 based
### constraint around "success" sample size 
pop_staticPhaseEnd = random.randint(30000, 40000)    ##Gen 75: year 1830, before whaling
pop_firstDecline = random.randint(100, 5000)         ##Gen 76: year 1855, whaling starts
pop_secondDecline= random.randint(36, 750)           ##Gen 77: year 1880, whaling continues
pop_thirdDecline= random.randint(36, 750)            ##Gen 78: year 1905, whaling continues
pop_bottleNeck = random.randint(36, 750)             ##Gen 79: year 1930, tohorā almost extinct
pop_firstRecovery= random.randint(36, 750)           ##Gen 80: year 1955, whales start recovering
pop_secondRecovery= random.randint(36, 750)          ##Gen 81: year 1980, there is hope
pop_currentGen= random.randint(500, 2500)            ##Gen 82: year 2005, current population doubles from last generation: numbers based on abundance estimate
pop_hopefulFuture= 4000                              ##Gen 83: year 2030, can we be this hopeful?
...
# def write_haploisofile(pop):
#     gen = pop.dvars().gen
#     sample = drawRandomSample(pop, sizes=sample_count)
#     with open(get_filename(f'haploiso_{gen}.txt'), 'w') as haplofile:
...
def write_haploisofile(pop, param=None):
    # Get generation number if param is not provided
    if param is None:
        param = pop.dvars().gen
    ...
    with open(get_filename(f'haploiso_{param}.txt'), 'w') as haplofile:
        print("sex, haplotype, carbon, nitrogen, feeding_ground, native_ground", file=haplofile)
        
        # Loop through individuals and write their data
        for individual in sample.individuals():
            genotype = individual.genotype()
            print(
                1 if individual.sex() == 1 else 0,
                ...
                individual.info('carbon'),
                individual.info('nitrogen'),
                individual.info('feeding_ground'),
                int(individual.info('native_breeding_ground')),
                file=haplofile, sep=' '
            )
...
def demo(gen, pop):
 ...
 #to keep from sampling into the projected populations, AG changed elif to else statement
 #all previous code just commented out
    else: #gen < currentGen:
...
def removeOverspill(pop):
 ...
 #to keep from sampling into the projected populations, AG changed elif to else statement
 #all previous code just commented out
    else: #gen == currentGen: #keep at current
...
# =============================================================================
# AG: added functions for reporting
# =============================================================================

def log_generation(pop):
    gen = pop.dvars().gen
    size = pop.popSize()
    with open(f"results/{SEED}/{SEED}_gen_summary.txt", "a") as f:
        f.write(f"{gen}\t{size}\n")
    return True  # required for PyOperator
...
def runSimulation(sub_population_size, minMatingAge, maxMatingAge, gen, mitochondrial_file, snp_file):
    ...
    pop.evolve(
        ...
        preOps = [
           ...
           ## commented out below to save space, 
           ## uncomment to explore SNP alleles for each gen
           #Exporter(
           #    format='csv',
           ...
           sim.PyOperator(func=write_haploisofile, at = genSteps)#param=staticPhaseEnd, step=staticPhaseEnd) #staticPhaseEnd #genSteps
        ],
        ...
        postOps = [
            ...
            sim.PyOperator(log_generation, at=range(0, gen)),
...
## get initial haplotype seed from r
## this is to locate the specific initial mtDNA data file
mtDNA_seed = os.environ.get("mtDNA_SEED")
DNA_SEED_DIR = Path("./mtDNA")
mtDNA_file = DNA_SEED_DIR / f"{mtDNA_seed}.mtDNA.txt"

print(f"mtDNA file used: {mtDNA_file}")
